(function($){
	...
	$.fn.youTubeEmbed = function(settings){
		...
		// The old version of this plugin used the SEARCH API, giving the id as a query 
		//   the url used was : http://gdata.youtube.com/feeds/api/videos?v=2&alt=jsonc
		var youtubeAPI = 'https://gdata.youtube.com/feeds/api/videos/' + settings.videoID + '?v=2&alt=jsonc';
		// The old version of this plugins used the SEARCH API giving the id as a query
		//   the call used was $.get(youtubeAPI,{'q': settings.videoID},function(response){
		$.get(youtubeAPI,{},function(response){
			...
			{				
				window.onYouTubePlayerReady = function(playerID){
					elements.player.cueVideoById(settings.videoID, 0, 'default');
					var levels = document.getElementById('video_'+playerID).getAvailableQualityLevels();
